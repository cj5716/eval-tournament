...
namespace ChessChallenge.UCI
{
    class UCIBot
    {
        ...
        void GoCommand(string[] args)
        {
            ...
            (API.Move move, int score) = bot.Think(apiBoard, timer);
            Console.WriteLine($"bestmove {move.ToString().Substring(7, move.ToString().Length - 8)} score cp {score}");
        }
